...
router.beforeEach(async (to, from) => {
  ...
  // 离开当前页面时是否需要添加当前页面缓存
  !new RegExp(/^\/redirect\//).test(from.path) && store.state.layout.tags.tagsList.some((v) => v.name === from.name) && !store.state.layout.tags.cachedViews.some((v) => v === from.name) && store.commit('layout/addCachedViews', { name: from.name, noCache: from.meta.noCache });
});
